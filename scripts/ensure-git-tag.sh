#!/usr/bin/env bash
# Create and push v{Version} if missing. Version comes from package.json (no v prefix).
set -euo pipefail

VERSION="${1:?Usage: ensure-git-tag.sh <semver from package.json>}"
TAG="v${VERSION}"
...
if git rev-parse "refs/tags/${TAG}" >/dev/null 2>&1; then
  ...
  echo "error: bump \"version\" in package.json — tags are immutable" >&2
  exit 1
fi
...
if [[ -n "${remote_sha}" ]]; then
  ...
  echo "error: bump \"version\" in package.json" >&2
  exit 1
fi

git tag -a "${TAG}" -m "Release ${VERSION} (from package.json via Woodpecker)"
git push origin "${TAG}"
