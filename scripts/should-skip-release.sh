#!/usr/bin/env bash
# Exit 0 when this commit should not run tag/publish (docs/CI only since last v{Version} tag).
set -euo pipefail
...
while IFS= read -r path; do
  [[ -z "${path}" ]] && continue
  case "${path}" in
    README.md|CONTRIBUTING.md|COVERAGE.md|TYPE-PERFILE.md|CHANGELOG.md|.gitignore|.woodpecker.yml|.npmrc|npmrc.consumer.example|scripts/*|docs/*|eslint-shared/*|vitest.config.ts|eslint.config.js|tsconfig.json|package-lock.json|*.tgz|*.tar.gz) ;;
    *)
      exit 1
      ;;
  esac
...
echo "[INFO] release skipped: no publishable changes since ${TAG} (${tag_sha:0:7})"
exit 0
