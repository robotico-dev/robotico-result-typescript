...
- Metal npm copy: `docker run --volumes-from` Woodpecker step (workspace bind-mount).
...
- Woodpecker release uses `network_mode: host` and loopback Verdaccio (`127.0.0.1:4873/npm/`) so publish lands on pest metal storage.
